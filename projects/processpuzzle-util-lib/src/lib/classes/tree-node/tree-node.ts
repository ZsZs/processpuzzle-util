...
export class TreeNode {
  ...
  public detach(): void {
    if ( !ObjectUtil.isNullOrUndefined( this._parent )) {
  ...
  public findDescendantChild( compareNodeProperty: ( subjectNode: TreeNode, propertyValue: any ) => boolean, propertyValue: any ): TreeNode | null {
    ...
    if ( compareNodeProperty( this, propertyValue )) {
      ...
      this._children.forEach( (child) => {
         ...
         if ( !ObjectUtil.isNullOrUndefined( node )) {
           foundNode = node;
         }
      });
    }
  ...
  public parents( includeCurrent?: boolean ): TreeNode[] {
    ...
    if ( !ObjectUtil.isNullOrUndefined( includeCurrent ) && includeCurrent ) {
      parents.push( this );
    }

    while ( !ObjectUtil.isNullOrUndefined( parent )) {
      parents.push( parent );
